...
import kmeans.Point;
import kmeans.Segments;

// /home/mosab/Desktop/input/1 /home/mosab/Desktop/output/ProcessWOH 10

//See the Covariance program in HIPI to know how to deal with the float image (generation, writing,etc.)
public class ProcessWOH extends Configured implements Tool {
			...
			for (Segments segments : values) {
				...
				for (Cluster cluster : segments.getClusters()) {
					for (Point point : cluster.getPoints()) {
						pixels[(point.getPexilLocation().getI() * segments.getWidth() + point.getPexilLocation().getJ())
								* 3 + 0] = (byte) colors.get(inc).getRed();
						pixels[(point.getPexilLocation().getI() * segments.getWidth() + point.getPexilLocation().getJ())
								* 3 + 1] = (byte) colors.get(inc).getGreen();
						pixels[(point.getPexilLocation().getI() * segments.getWidth() + point.getPexilLocation().getJ())
								* 3 + 2] = (byte) colors.get(inc).getBlue();
					}
					inc++;
				}

				Configuration conf = context.getConfiguration();
				Path outputPath = new Path(conf.get("bundler.outpath") + "/" + key + ".jpeg");
				...
				File fgen = new File(conf.get("bundler.outpath") + "/" + key + ".jpeg");
				OutputStream os = new FileOutputStream(fgen);
				//
				...
				final WritableRaster writableRaster = Raster.createInterleavedRaster(dataBuffer, segments.getWidth(),
						segments.getHeight(), segments.getWidth() * 3, 3, offsets, null);
	...
	public int run(String[] args) throws Exception {
		...
		if (args.length < 2) {
			System.out.println("Usage: FirstProgram <input directory> <output directory>  <number of download nodes>");
			System.exit(0);
		}
		...
		Job job = Job.getInstance(conf, "ProcessWOH");
		job.setJarByClass(ProcessWOH.class);
		job.setMapperClass(MeanMapper.class);
		job.setReducerClass(MeanReducer.class);

		job.setInputFormatClass(ProcessInputFormatWOH.class);
		...
		ProcessInputFormatWOH.setInputPaths(job, new Path(args[0]));
	...
	public static void main(String[] args) throws Exception {
		...
		int res = ToolRunner.run(conf,new ProcessWOH(), args);
		System.exit(res);
	}
