package mosab.process.bundler.ReadFromFolderSendingFileList;
...
public class Bundler extends Configured implements Tool {
	...
	public static class BundlerMapper extends Mapper<IntWritable, TextArrayWritable, BooleanWritable, Text> {
		...
		// temporary HIB.
		@Override
		public void map(IntWritable key, TextArrayWritable value, Context context)
			...
			for (int i = 0; i < paths.length; i++) {
				...
				try {
					...
					try {
						Path path = new Path(paths[i].toString());
						System.err.println("Accessing " + path.toString());
						in = fs.open(path);
						java.nio.file.Path p = Paths.get(paths[i].toString());
					...
					if (type != null && (type.compareTo("image/jpeg") == 0 || type.compareTo("image/png") == 0
						...
						System.err.println(
								"Unsupported image format [" + type + "], not added to HIB: " + paths[i].toString());
					}
