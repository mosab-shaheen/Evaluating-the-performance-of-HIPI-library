package mosab.process.bundler.ReadFromFile;
 ...
 * <br />
 * bundler.jar /path/to/filelist.txt /path/to/output.hib 10 <br />
 * /home/mosab/Desktop/filelist.txt /home/mosab/Desktop/HIB/output.hib 10
 * 
 * <br />
...
public class Bundler extends Configured implements Tool {
	...
	public static class BundlerMapper extends Mapper<IntWritable, Text, BooleanWritable, Text> {

		// Download images at the list of input URLs and store them in a
		// temporary HIB.
		@Override
		public void map(IntWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			// Setup buffered reader to allow processing this string line by
			// line.
			...
			while ((fpath = reader.readLine()) != null) {
				...
				try {
					...
					try {
						...
						System.err.println("Accessing " + path.toString());
						in = fs.open(path);
						java.nio.file.Path p = Paths.get(fpath);
						System.out.println(path + " : " + Files.probeContentType(p));
						type = Files.probeContentType(p);
					} catch (Exception e) {
						System.err.println("Error while trying to access: " + fpath);
						continue;
					}

					// Check that image format is supported, header is parsable,
					// and add to HIB if so
					if (type != null && (type.compareTo("image/jpeg") == 0 || type.compareTo("image/png") == 0
							|| type.compareTo("image/ppm") == 0 || type.compareTo("image/tiff") == 0)) {
						...
						if (header == null) {
							System.err.println("Failed to parse header, not added to HIB: " + fpath);
						} else {

							// Passed header decode test, so reset to beginning
							// of stream
							bis.reset();
							...
							System.err.println("Added to HIB: " + fpath);

						}
					} else {
						System.err.println("Unsupported image format [" + type + "], not added to HIB: " + fpath);
					}

				} catch (Exception e) {
					e.printStackTrace();
					System.err.println("Encountered error while trying to access: " + fpath);
					try {
						Thread.sleep(1000);
			...
			try {
				...
				// Output key/value pair to reduce layer consisting of boolean
				// and path to HIB
				context.write(new BooleanWritable(true), new Text(hib.getPath().toString()));
	...
	public static class BundlerReducer extends Reducer<BooleanWritable, Text, BooleanWritable, Text> {
		...
		public void reduce(BooleanWritable key, Iterable<Text> values, Context context)
			...
			if (key.get()) {
				...
				for (Text tempString : values) {
					...
					// Emit output key/value pair indicating temporary HIB has
					// been processed
	...
	public int run(String[] args) throws Exception {
		...
		if (args.length != 3) {
			System.out.println(
					"Usage: bundler <input text file with list of files paths> <output HIB> <number of download nodes>");
			System.exit(0);
		}
